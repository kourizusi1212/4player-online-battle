...
create.onclick=()=>{let r=String(Math.floor(1000+Math.random()*9000));ws.send(JSON.stringify({type:"create",name:nameI.value||"Player",room:r}))};
...
copy.onclick=async()=>{try{await navigator.clipboard.writeText(room);copymsg.textContent="コピーしました！"}catch{}};
