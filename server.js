...
wss.on("connection",ws=>{let r=null,p=null;ws.on("message",raw=>{let m;try{m=JSON.parse(raw)}catch{return}
 if((m.type==="create"||m.type==="join")&&!p){const id=String(m.room||"").replace(/\D/g,"");if(!/^\d{4}$/.test(id))return send(ws,{type:"error",msg:"4桁の部屋番号を入力してください"});if(m.type==="create"&&rooms.has(id))return send(ws,{type:"error",msg:"その部屋番号は使用中です"});if(m.type==="join"&&!rooms.has(id))return send(ws,{type:"error",msg:"その部屋は存在しません"});
 r=m.type==="create"?getRoom(id,m.mapKey):rooms.get(id);
 if(r.started)return send(ws,{type:"error",msg:"その部屋は対戦中です"});if(r.players.size>=4)return send(ws,{type:"error",msg:"部屋が満員です"});const i=r.players.size,s=spawns[i];p={id:Math.random().toString(36).slice(2,8),ws,name:String(m.name||"Player").slice(0,12),x:s[0]+.5,y:s[1]+.5,a:0,hp:100,score:0,kills:0,deaths:0,alive:true,color:colors[i],weapon:"pistol",ammo:12,reloading:false,lastShot:0,speedBoostUntil:0};r.players.set(p.id,p);if(!r.host)r.host=p.id;send(ws,{type:"joined",id:p.id,room:id,map:getMap(r),mapKey:r.mapKey,host:r.host===p.id,items:r.items});broadcastState(r,true);return}
 if(!p||!r)return;
